...
Rust firmware for a Raspberry Pi Pico that sends serial data over USB.
...
```text
target/thumbv6m-none-eabi/release/pico-serial
```
...
1. Convert the release build to a UF2 file:
   ...
   ```sh
   elf2uf2-rs target/thumbv6m-none-eabi/release/pico-serial pico-serial.uf2
   ```
...
7. Copy the firmware onto that drive:
   ...
   ```sh
   cp pico-serial.uf2 /Volumes/RPI-RP2/
   ```

   Linux:

   ```sh
   cp pico-serial.uf2 /media/$USER/RPI-RP2/
   ```

   Windows PowerShell:

   ```powershell
   Copy-Item .\pico-serial.uf2 E:\
   ```
...
should serial every half second.
